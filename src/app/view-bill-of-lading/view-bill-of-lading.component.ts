...
import { BillOfLading, Consignment} from '../models/consignment';
import 'rxjs/add/operator/switchMap'; 
import {LoginAuthenticationService } from '../services/login-authentication.service';

@Component({
  selector: 'app-view-bill-of-lading',
  templateUrl: './view-bill-of-lading.component.html',
  styleUrls: ['./view-bill-of-lading.component.css']
})
export class ViewBillOfLadingComponent implements OnInit {

  private bolData : string;
  private carrierVerified : boolean = false;
  private carrierVerificationSuccess : boolean = false;
  ...
  ngOnInit() {
     this.carrierVerified=false;
     this.carrierVerificationSuccess=false;
     this.createForm();
  ...
  this.fileUploadService.verifyDSFile(formData).subscribe(
          response => {
              console.log("res received verifyDSFile service" + JSON.stringify(response));
               this.carrierVerified = true;
                if (response !=null && response.success) {
                  this.carrierVerificationSuccess = true;
...
getBillOfLading(){
        this.route.params
          .switchMap((params: Params) => this.contractManagementService.getBillOfLadingDetails(params['id']))
          .subscribe(
            response => {
                if(response && response.bol){
                  let bolData = <BillOfLading>response.bol;
